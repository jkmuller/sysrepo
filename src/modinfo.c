        ...
        struct sr_mod_info_s *mod_info)
{
    ...
    if (prev_mod_type < MOD_INFO_INV_DEP) {
        ...
        for (i = 0; i < shm_mod->data_dep_count; ++i) {
            ...
            /* find ly module */
            ly_mod = ly_ctx_get_module_implemented(ly_mod->ctx, mod_info->conn->ext_shm.addr + dep_mod->name);
...
sr_oper_data_trim_r(struct lyd_node **data, struct lyd_node *sibling, sr_get_oper_options_t opts)
{
    ...
    LY_LIST_FOR_SAFE(sibling, next, elem) {
        ...
        sr_oper_data_trim_r(data, lyd_node_children(elem, 1), opts);

        if ((elem->schema->flags & LYS_CONFIG_W) && (opts & SR_OPER_NO_CONFIG) && !lyd_node_children(elem, 1)) {
        ...
        const struct lyd_node *data, sr_sid_t *sid, uint32_t timeout_ms, sr_error_info_t **cb_error_info)
{
    ...
    for (i = 0; i < shm_dep_count; ++i) {
        if (shm_deps[i].type == SR_DEP_INSTID) {
            if (data) {
                if (lyd_find_xpath(data, conn->ext_shm.addr + shm_deps[i].xpath, &set)) {
                    sr_errinfo_new_ly(&err_info, conn->ly_ctx);
                    goto cleanup;
                }
            } else {
            ...
            if (set->count) {
                ...
                for (j = 0; j < set->count; ++j) {
                    mod_name = sr_get_first_ns(SR_LY_TERM_VALUE(set->dnodes[j]));
    ...
    for (i = 0; i < dep_set->count; ++i) {
        dep_mod = (sr_mod_t *)dep_set->objs[i];

        ly_mod = ly_ctx_get_module_implemented(mod_info->conn->ly_ctx, conn->ext_shm.addr + dep_mod->name);
