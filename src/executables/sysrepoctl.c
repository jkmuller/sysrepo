        ...
        size_t *list_count)
{
    ...
    LY_LIST_FOR(LYD_CHILD(sr_data), module) {
        ...
        *list = realloc(*list, (*list_count + 1) * sizeof **list);
        ...
        child = lyd_node_children(module, 0);
        assert(!strcmp(child->schema->name, "name"));
        cur_item->name = strdup(((struct lyd_node_term *)child)->value.canonical_cache);

        child = child->next;
        if (!strcmp(child->schema->name, "revision")) {
            cur_item->revision = strdup(((struct lyd_node_term *)child)->value.canonical_cache);
        }
        ...
        LY_LIST_FOR(child, child) {
            if (!strcmp(child->schema->name, "replay-support")) {
                ...
                str = ((struct lyd_node_term *)child)->value.canonical_cache;
