        ...
        uint32_t max_depth, int wd_opt, int timeout_s)
{
    ...
    /* print exported data */
    lyd_print_file(file ? file : stdout, data, format, LYD_PRINT_FORMAT | LYD_PRINT_WITHSIBLINGS | wd_opt);
    lyd_free_all(data);
...
op_rpc(sr_session_ctx_t *sess, const char *file_path, const char *editor, LYD_FORMAT format, int wd_opt, int timeout_s)
{
    ...
    if (node) {
        lyd_print_file(stdout, output, format, LYD_PRINT_FORMAT | wd_opt);
    }
    lyd_free_all(output);
