...
sr_val_ly2sr(const struct lyd_node *node, sr_val_t *sr_val)
{
    ...
    case LYS_LEAFLIST:
        ...
        case LY_TYPE_BINARY:
            ...
            sr_val->data.binary_val = strdup(leaf->value.canonical_cache);
            SR_CHECK_MEM_GOTO(!sr_val->data.binary_val, err_info, error);
            break;
        case LY_TYPE_BITS:
            sr_val->type = SR_BITS_T;
            sr_val->data.bits_val = strdup(leaf->value.canonical_cache);
        ...
        case LY_TYPE_ENUM:
            ...
            sr_val->data.enum_val = strdup(leaf->value.canonical_cache);
            SR_CHECK_MEM_GOTO(!sr_val->data.enum_val, err_info, error);
            break;
        case LY_TYPE_IDENT:
            sr_val->type = SR_IDENTITYREF_T;
            sr_val->data.identityref_val = strdup(leaf->value.canonical_cache);
            SR_CHECK_MEM_GOTO(!sr_val->data.identityref_val, err_info, error);
            break;
        case LY_TYPE_INST:
            sr_val->type = SR_INSTANCEID_T;
            sr_val->data.instanceid_val = strdup(leaf->value.canonical_cache);
        ...
        case LY_TYPE_STRING:
            ...
            sr_val->data.string_val = strdup(leaf->value.canonical_cache);
    ...
    case LYS_ANYDATA:
        ...
        case LYD_ANYDATA_LYB:
            ...
            if (lyd_parse_data_mem(LYD_NODE_CTX(node), any->value.mem, LYD_LYB, LYD_PARSE_STRICT, 0, &tree)) {
        ...
        case LYD_ANYDATA_DATATREE:
            lyd_print_mem(&ptr, any->value.tree, LYD_XML, LYD_PRINT_FORMAT | LYD_PRINT_WITHSIBLINGS);
            break;
        }
...
sr_val_sr2ly(struct ly_ctx *ctx, const char *xpath, const char *val_str, int dflt, int output, struct lyd_node **root)
{
    ...
    opts = LYD_NEWOPT_UPDATE | (output ? LYD_NEWOPT_OUTPUT : 0);
...
sr_lyd_dup(const struct lyd_node *src_parent, uint32_t depth, struct lyd_node *trg_parent)
{
    ...
    while (src_child) {
        if (lyd_dup_single(src_child, NULL, LYD_DUP_WITH_FLAGS, &trg_child)) {
            sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(src_parent));
            return err_info;
        }
        ...
        const struct lys_module *ly_mod)
{
    ...
    for (src = src_sibling; src; src = src->next) {
        ...
        if (lyd_new_inner(parent, src->schema->module, src->schema->name, &node)) {
            sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(src));
            return err_info;
        }
...
sr_lyd_dup_enabled_xpath(const struct lyd_node *data, char **xpaths, uint16_t xp_count, struct lyd_node **new_data)
{
    ...
    for (i = 0; i < xp_count; ++i) {
        ...
        /* merge into one set */
        if (set) {
            if (ly_set_merge(set, cur_set, 0, NULL) == -1) {
...
sr_ly_anydata_value_str(const struct lyd_node *any, char **value_str)
{
    ...
    *value_str = NULL;
    ...
    case LYD_ANYDATA_LYB:
        ...
        if (lyd_parse_data_mem(LYD_NODE_CTX(a), a->value.mem, LYD_LYB, LYD_PARSE_STRICT, 0, &tree)) {
            sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(a));
            return err_info;
        }
        dynamic = 1;
        break;
    ...
    if (tree) {
        /* print into a string */
        if (lyd_print_mem(value_str, tree, LYD_XML, LYD_PRINT_WITHSIBLINGS)) {
        ...
        mode_t create_mode)
{
    ...
    if (lyd_print_fd(fd, mod_data, LYD_LYB, LYD_PRINT_WITHSIBLINGS)) {
        sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(mod_data));
