        ...
        int keep_cur_child)
{
    ...
    if ((!cur_pid && !cur_conn_ptr) || (cur_pid != getpid()) || (cur_conn_ptr != conn_ptr)) {
        ...
        if (lyd_new_meta(diff_node, NULL, SR_YANG_MOD ":pid", pid_str, NULL)) {
            sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(diff_node));
            return err_info;
        }
        ...
        LY_LIST_FOR(lyd_node_children(diff_node, 1), child) {
...
sr_lyd_diff_merge_cb(const struct lyd_node *src_node, struct lyd_node *trg_node, void *oper_conn)
{
    ...
    LY_LIST_FOR(lyd_node_children(trg_node, 1), child) {
        ...
        struct lyd_node **match)
{
    ...
    if (lyrc == LY_ENOTFOUND) {
        ...
        sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(llist));
        return err_info;
    }
        ...
        const char *key_or_value, int dflt_ll_skip, struct lyd_node **match_p, int *val_equal_p)
{
    ...
    if ((op == EDIT_PURGE) && (edit_node->schema->nodetype & (LYS_LIST | LYS_LEAFLIST))) {
        ...
        if (lyrc && (lyrc != LY_ENOTFOUND)) {
            sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(edit_node));
            return err_info;
        }
        ...
        const char **key_or_value)
{
    ...
    *op = parent_op;
    ...
    LY_LIST_FOR(edit_node->meta, meta) {
        val_str = meta->value.canonical_cache;
        ...
        enum insert_val insert, const char *key_or_value)
{
    ...
    if (!*first_node) {
        ...
        if (lyd_insert_child(parent_node, new_node)) {
            sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(parent_node));
            return err_info;
        }
        return NULL;
    }
...
sr_edit_create_userord_predicate(const struct lyd_node *llist)
{
    ...
    if (llist->schema->nodetype == LYS_LEAFLIST) {
        pred = strdup(SR_LY_TERM_VALUE(llist));
        return pred;
    }
    ...
    pred_len = 0;
    pred = NULL;
    for (key = lyd_node_children(llist, 0); key && (key->schema->flags & LYS_KEY); key = key->next) {
        key_len = 1 + strlen(key->schema->name) + 2 + strlen(SR_LY_TERM_VALUE(key)) + 2;
        pred = sr_realloc(pred, pred_len + key_len + 1);
        if (!pred) {
            return NULL;
        }

        sprintf(pred + pred_len, "[%s='%s']", key->schema->name, SR_LY_TERM_VALUE(key));
        pred_len += key_len;
    }
...
sr_edit_set_oper(struct lyd_node *edit, const char *op)
{
    ...
    if (lyd_new_meta(edit, NULL, attr_full_name, op, NULL)) {
        sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(edit));
        return err_info;
    }
        ...
        LY_LIST_FOR(edit->meta, meta) {
            if (!strcmp(meta->name, "operation")) {
                if (!strcmp(meta->annotation->module->name, SR_YANG_MOD)
                    ...
                    return sr_edit_str2op(meta->value.canonical_cache);
                }
            }
        }
...
sr_edit_diff_set_origin(struct lyd_node *node, const char *origin, int overwrite)
{
    ...
    if (lyd_new_meta(node, NULL, "ietf-origin:origin", origin, NULL)) {
        sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(node));
        return err_info;
    }
...
sr_edit_delete_set_cont_dflt(struct lyd_node *parent)
{
    ...
    LY_LIST_FOR(lyd_node_children(parent, 0), iter) {
...
sr_diff_is_redundant(struct lyd_node *diff)
{
    ...
    child = lyd_node_children(diff, 1);
    yang_mod = ly_ctx_get_module_latest(LYD_NODE_CTX(diff), "yang");
    assert(yang_mod);
        ...
        if (lysc_is_userordered(elem->schema)) {
            ...
            if (elem->schema->nodetype == LYS_LIST) {
                if (lyd_new_meta(elem, NULL, "yang:key", sibling_before_val, NULL)) {
                    sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(elem));
                }
            } else {
    ...
    if (val_equal) {
        *next_op = EDIT_NONE;
        ...
        case LYS_LEAF:
            ...
            prev_val = strdup(SR_LY_TERM_VALUE(match_node));
            SR_CHECK_MEM_RET(!prev_val, err_info);
            prev_dflt = match_node->flags & LYD_DEFAULT;

            /* modify the node */
            lyrc = lyd_change_term(match_node, SR_LY_TERM_VALUE(edit_node));
        ...
        case LYS_ANYDATA:
            ...
            if (lyd_any_copy_value(match_node, &any->value, any->value_type)) {
                free(prev_val);
                sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(match_node));
                return err_info;
            }
...
reapply:
    ...
    if (flags & EDIT_APPLY_REPLACE_R) {
        ...
        LY_LIST_FOR_SAFE(lyd_node_children(match, 1), next, child) {
            if (child->flags & LYD_DEFAULT) {
                continue;
            }

            if ((err_info = sr_edit_find(lyd_node_children(edit_node, 1), child, EDIT_DELETE, 0, NULL, 0, &edit_match, NULL))) {
                return err_info;
            }
    ...
    LY_LIST_FOR(lyd_node_children(edit_node, 1), child) {
        if (flags & EDIT_APPLY_CHECK_OP_R) {
...
sr_diff_op(const struct lyd_node *diff_node, enum edit_op *op, const char **key_or_value)
{
    ...
    for (diff_parent = diff_node; diff_parent; diff_parent = LYD_PARENT(diff_parent)) {
        LY_LIST_FOR(diff_parent->meta, meta) {
            if (!strcmp(meta->name, "operation")) {
                if (!strcmp(meta->annotation->module->name, SR_YANG_MOD)) {
                    assert(!strcmp(meta->value.canonical_cache, "none"));
                    *op = EDIT_NONE;
                    break;
                } else if (!strcmp(meta->annotation->module->name, "ietf-netconf")) {
                    if (!strcmp(meta->value.canonical_cache, "create")) {
                        *op = EDIT_CREATE;
                    } else if (!strcmp(meta->value.canonical_cache, "delete")) {
                        *op = EDIT_DELETE;
                    } else if (!strcmp(meta->value.canonical_cache, "replace")) {
    ...
    *key_or_value = NULL;
    if (lysc_is_userordered(diff_node->schema)) {
        if ((*op == EDIT_CREATE) || (*op == EDIT_REPLACE)) {
            ...
            *key_or_value = meta->value.canonical_cache;
        }
    }
...
next_iter_r:
    ...
    if (!lyd_node_children(diff_node, 1)) {
        /* we are done */
        return NULL;
    }

    /* update diff recursively */
    LY_LIST_FOR_SAFE(lyd_node_children(diff_node, 1), next, diff_child) {
        if ((err_info = sr_diff_update_r(lyd_node_children(match, 1), diff_child, diff_root))) {
            return err_info;
        }
    }
    if ((op == EDIT_NONE) && !lyd_node_children(diff_node, 1)) {
        ...
        const char *origin, int isolate)
{
    ...
    if (!op) {
        ...
        for (parent = LYD_PARENT(node); parent; node = parent, parent = LYD_PARENT(parent)) {
            ...
            for (sibling = lyd_node_children(parent, 1); sibling; sibling = sibling->next) {
                if (sibling == node) {
                    continue;
                }
    ...
    while (*idx < set->count) {
        *node = set->dnodes[*idx];
        ...
        if (((*node)->schema->flags & LYS_KEY) && lysc_is_userordered((*node)->parent->schema)
                && (((char *)meta->value.canonical_cache)[0] == 'r')) {
...
sr_diff_del_conn(struct lyd_node **diff, sr_conn_ctx_t *conn, pid_t pid)
{
    ...
    if (lyd_find_xpath(*diff, xpath, &set)) {
        sr_errinfo_new_ly(&err_info, LYD_NODE_CTX(*diff));
        goto cleanup;
    }
