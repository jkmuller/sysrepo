    ...
    LY_LIST_FOR(LYD_CHILD(sr_mods), sr_mod) {
        ...
        LY_LIST_FOR(LYD_CHILD(sr_mod), sr_child) {
            ...
            if (!strcmp(sr_child->schema->name, "name")) {
                /* a string */
                shm_size += sr_strshmlen(SR_LY_TERM_VALUE(sr_child));
                    ...
                    if (!strcmp(sr_dep->schema->name, "inst-id")) {
                        LY_LIST_FOR(LYD_CHILD(sr_dep), sr_instid) {
                            if (!strcmp(sr_instid->schema->name, "xpath")) {
                                /* a string */
                                shm_size += sr_strshmlen(SR_LY_TERM_VALUE(sr_instid));
                            }
                        }
                    }
                }
                ...
                LY_LIST_FOR(LYD_CHILD(sr_child), sr_op_dep) {
                    if (!strcmp(sr_op_dep->schema->name, "xpath")) {
                        ...
                        shm_size += sr_strshmlen(SR_LY_TERM_VALUE(sr_op_dep));
        ...
        sr_mod_data_dep_t *shm_deps, uint32_t *dep_i, char **ext_cur)
{
    ...
    LY_LIST_FOR(LYD_CHILD(sr_dep_parent), sr_dep) {
        ...
        if (!strcmp(sr_dep->schema->name, "module")) {
            ...
            ref_shm_mod = sr_shmmain_find_module(shm_main, ext_shm_addr, SR_LY_TERM_VALUE(sr_dep), 0);
            ...
            LY_LIST_FOR(LYD_CHILD(sr_dep), sr_instid) {
                if (!strcmp(sr_instid->schema->name, "xpath")) {
                    /* copy xpath */
                    shm_deps[*dep_i].xpath = sr_shmstrcpy(ext_shm_addr, SR_LY_TERM_VALUE(sr_instid), ext_cur);
                } else if (!strcmp(sr_instid->schema->name, "default-module")) {
                    /* copy module name offset */
                    ref_shm_mod = sr_shmmain_find_module(shm_main, ext_shm_addr, SR_LY_TERM_VALUE(sr_instid), 0);
                    SR_CHECK_INT_RET(!ref_shm_mod, err_info);
                    shm_deps[*dep_i].module = ref_shm_mod->name;
                }
            }
...
sr_shmmain_add_modules(char *ext_shm_addr, struct lyd_node *first_sr_mod, sr_mod_t *first_shm_mod, off_t *ext_end)
{
    ...
    LY_LIST_FOR(first_sr_mod, first_sr_mod) {
        ...
        LY_LIST_FOR(LYD_CHILD(first_sr_mod), sr_child) {
            if (!strcmp(sr_child->schema->name, "name")) {
                /* copy module name */
                first_shm_mod->name = sr_shmstrcpy(ext_shm_addr, SR_LY_TERM_VALUE(sr_child), &ext_cur);
            } else if (!strcmp(sr_child->schema->name, "revision")) {
                /* copy revision */
                strcpy(first_shm_mod->rev, SR_LY_TERM_VALUE(sr_child));
        ...
        LY_LIST_FOR(LYD_CHILD(first_sr_mod), sr_child) {
            if (!strcmp(sr_child->schema->name, "enabled-feature")) {
                /* copy feature name */
                shm_features[feat_i] = sr_shmstrcpy(ext_shm_addr, SR_LY_TERM_VALUE(sr_child), &ext_cur);
    ...
    LY_LIST_FOR(first_sr_mod, first_sr_mod) {
        ...
        LY_LIST_FOR(LYD_CHILD(first_sr_mod), sr_child) {
            if (!strcmp(sr_child->schema->name, "data-deps")) {
                ...
                ref_shm_mod = sr_shmmain_find_module(shm_main, ext_shm_addr, SR_LY_TERM_VALUE(sr_child), 0);
                SR_CHECK_INT_RET(!ref_shm_mod, err_info);
                ...
                LY_LIST_FOR(LYD_CHILD(sr_child), sr_op) {
                    if (!strcmp(sr_op->schema->name, "xpath")) {
                        /* copy xpath name */
                        shm_op_deps[op_dep_i].xpath = sr_shmstrcpy(ext_shm_addr, SR_LY_TERM_VALUE(sr_op), &ext_cur);
