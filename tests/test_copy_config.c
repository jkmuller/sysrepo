...
copy_userord_thread(void *arg)
{
    ...
    assert_string_equal(((struct lyd_node_term *)LYD_CHILD(node))->value.canonical_cache, "a");
    node = node->next;
    assert_string_equal(node->schema->name, "l2");
    assert_string_equal(((struct lyd_node_term *)LYD_CHILD(node))->value.canonical_cache, "b");
    node = node->next;
    assert_string_equal(node->schema->name, "ll2");
    assert_string_equal(((struct lyd_node_term *)node)->value.canonical_cache, "2");
    node = node->next;
    assert_string_equal(node->schema->name, "ll2");
    assert_string_equal(((struct lyd_node_term *)node)->value.canonical_cache, "1");
...
replace_thread(void *arg)
{
    ...
    assert_string_equal(((struct lyd_node_term *)LYD_CHILD(config))->value.canonical_cache, "c");

    assert_string_equal(config->next->schema->name, "l1");
    assert_string_equal(((struct lyd_node_term *)LYD_CHILD(config->next))->value.canonical_cache, "a");

    assert_string_equal(config->next->next->schema->name, "l1");
    assert_string_equal(((struct lyd_node_term *)LYD_CHILD(config->next->next))->value.canonical_cache, "b");
